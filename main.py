...
word_list_sea = ["яхта", "парус", "сочи", "рыба", "якорь", "чайки", "медузы"]
word_list_mat = ["бургер", "пицца", "суши", "паста", "суп", "салат", "десерт", "закуска"]
word_list_science = ["математика", "физика", "химия", "русский", "философия", "теоретическая механика", "информатика",
                     "география"]


def get_word():
    if f == 1:
        word = random.choice(word_list_sea)
        return word.upper()
    elif f == 2:
        word = random.choice(word_list_mat)
        return word.upper()
    elif f == 3:
        word = random.choice(word_list_science)
        return word.upper()
...
while again.lower() == 'д':
    print('выберите категорию (1-3): (море, еда, науки)')
    f = int(input())
    word = get_word()
    play(word)
